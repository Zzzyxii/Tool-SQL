...
class EnvironmentManager:
    ...
    def get_new_environment_data(self, ability: str, 
        ...
        if config is None:
            ...
            config = dict(config)
        config.setdefault("ability", ability)
        config.setdefault("ability_hint", ability)
    ...
    def _extract_environment_type(self, ability: str) -> Optional[str]:
        ...
        if "apigen" in ability_lower or "airline_sql" in ability_lower or "taubench_airline_sql" in ability_lower:
            return "sqlbench_apigen"
        if "retail" in ability_lower:
            return "retail"
        elif "airline" in ability_lower:
            return "airline"
        elif "bowling" in ability_lower:
            return "bowling"
        elif "cosql" in ability_lower:
            return "cosql"
        else:
            return None
