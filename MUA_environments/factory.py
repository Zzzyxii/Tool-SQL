...
from .sqlbench import SQLBenchBowlingEnvironment, SQLBenchApigenEnvironment
...
class EnvironmentFactory:
    ...
    def _register_default_environments(self) -> None:
        ...
        global_environment_registry.register_environment("bowling", SQLBenchBowlingEnvironment)
        global_environment_registry.register_environment("sqlbench_apigen", SQLBenchApigenEnvironment)
        # Register COSQL multi-db environment
        try:
            from .sqlbench.cosql.environment import SQLBenchCoSQLEnvironment
            global_environment_registry.register_environment("cosql", SQLBenchCoSQLEnvironment)
        except Exception as exc:  # pragma: no cover
            # Non-fatal: training can proceed without COSQL if not needed
            import logging
            logging.getLogger(__name__).warning("Failed to register COSQL environment: %s", exc)
