...
#BASE_DIR="/checkpoint_mount/slz-fc-rebuttalz-complexxx/apigen/CKPT/Qwen3-mua-grpo-b16_mb4_n8_nothink_apigen_Rsql_compute_score_T1.2_apigen8b-complex-withkb/global_step_350"
BASE_DIR="/checkpoint_mount/slz-fc-rebuttalz-cos-continuejudge/cosql/CKPT/Qwen3-cosql-grpo-b16_mb4_n8_nothink_cosql_Rsql_compute_score_T1.2_cosql8b/global_step_60"
MODEL_NAME= "" #"global_step_100" # MODEL_NAME-SUFFIX
...
# Check for direct actor directory (Single Checkpoint Mode)
if [ -d "$MODEL_DIR/actor" ]; then
    echo "Detected single checkpoint structure in $MODEL_DIR"
    local_dir="$MODEL_DIR/actor"
    target_dir="/global_data/sft_intern/slz/zyx/models/continue_8bv2_60"
    ...
    echo "Processing: $local_dir -> $target_dir"
    ...
    if [ $? -eq 0 ]; then
        echo "Successfully merged to $target_dir"
        exit 0
    else
        echo "Error merging checkpoint"
        exit 1
    fi
fi
