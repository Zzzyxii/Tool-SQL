    ...
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                if not var_vals:
                    continue
                # For non-numeric vars (e.g., strings like "pred" or dicts like "per_sample"), skip
                if isinstance(var_vals[0], str) or isinstance(var_vals[0], dict):
                    continue

                # Filter out None/NaN and non-numeric entries safely
                numeric_indices = []
                numeric_vals = []
                for idx, v in enumerate(var_vals):
                    try:
                        # Accept bool/int/float and numpy number types
                        if v is None:
                            continue
                        if isinstance(v, (bool, int, float)) or (hasattr(np, 'number') and isinstance(v, np.number)):
                            # Exclude NaNs
                ...
                # If nothing numeric remains, skip this variable
                if not numeric_vals:
                    continue
                ...
                if n_resps > 1:
                    ...
                    ns = []
                    # Include 1 to 5 if they are <= n_resps
                    for k in range(1, 6):
                        if k <= n_resps:
                            ns.append(k)
                    
                    # Then powers of 2 starting from 8
                    n = 8
                    while n < n_resps:
                        ns.append(n)
                        n *= 2
                    
                    # Always include n_resps
                    ...
                    for n in ns:
                        ...
                        if var2vals.get("pred", None) is not None:
                            # Align majority-vote pairs to valid numeric indices only
                            preds = var2vals["pred"]
                            vote_data = []
                            for i, val in zip(numeric_indices, numeric_vals):
                                if i < len(preds):
                                    vote_data.append({"val": val, "pred": preds[i]})
