...
class ToolCompletionCallback(CompletionCallback):
    ...
    async def __call__(self, messages: List[Dict[str, str]], completions: ChatCompletion, info: Dict[str, Any]):
        raw_msg = completions.choices[0].message
        # message from completion can be Pydantic model, dict, or plain string depending on server
        if isinstance(raw_msg, str):
            message = {"role": "assistant", "content": raw_msg}
        else:
            try:
                # Pydantic/BaseModel-like object
                message = raw_msg.model_dump(exclude_unset=True, exclude_none=True)
            except Exception:
                # Fallback: if it's already a dict-like, use it; otherwise stringify
                if isinstance(raw_msg, dict):
                    message = raw_msg
                else:
                    message = {"role": "assistant", "content": str(raw_msg)}
