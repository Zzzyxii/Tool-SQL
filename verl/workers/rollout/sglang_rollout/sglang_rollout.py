...
from verl.third_party.sglang import patches as sglang_patches  # noqa: F401
...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        **kwargs,
        ...
        # Load terminal_tools and stop_word from manager with config fallback
        multi_turn_config = config.multi_turn if hasattr(config, 'multi_turn') else {}
        self.terminal_tools = environment_manager.get_terminal_tools(multi_turn_config) 
        self.stop_word = environment_manager.get_stop_word(multi_turn_config)
    ...
    def _initialize_tools(self, config, tokenizer):
        ...
        tool_schemas = [tool.get_openai_tool_schema().model_dump(exclude_none=True) for tool in tool_list]
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            print(f"[DEBUGGG] sglang input_ids: {idx_list}")  # Debug: print input before sglang request
            if self._tp_rank == 0:
    ...
    async def _server_by_close_chat(self, messages, model='slz-verl-qwen3-32b', app_key=None, url=None):
        api_key =  os.environ.get('API_KEY', app_key)
        if not api_key:
            api_key = "EMPTY"
        
        base_url = os.environ.get('BASE_URL', url)
        if not base_url:
            base_url = "http://slz-nohallu-z32b.bcloud.hb1a-h20.ml.baichuan-inc.com/v1"
        ...
        while True:
            try:
                ...
                # 修复：使用 client 而不是 openai
                # 增大 max_tokens 以防止思考过程挤占回答空间？
                ...
                if hasattr(response, 'model_dump'):
                    ...
                    try:
                        ...
                        answer = {"raw_response": str(response)}
                        
                # 新增：检查是否是 HTML 响应
                if isinstance(answer, dict) and 'raw_response' in answer:
                    raw_response = answer['raw_response']
                    if '<!doctype html>' in raw_response.lower() or '<html>' in raw_response.lower():
                        raise Exception(f"API returned HTML page instead of JSON response. Check API endpoint and authentication. Response: {raw_response[:200]}...")
                    # 修复结束
                return answer
                ...
                if retry_times > 10:
                    break
                print(f" {model} rate limited or failed (attempt {retry_times}), retrying at next minute...")
                await asyncio.sleep(60)
        raise Exception(f"{model} no return result")
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        tool_call_count = 0
        tool_call_times_by_tool = {tool_name: 0 for tool_name in self._tool_map}
        current_turns = 0
        while current_turns < self.config.multi_turn.max_turns: 
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                await self._handle_pending_state(_req)
                _req.state = AsyncRolloutRequestStateEnum.RUNNING
            elif _req.state == AsyncRolloutRequestStateEnum.USER:
                try:
                    # # === 新增：同步上下文逻辑 ===
                    # # 确保 Assistant 刚才的思考、工具调用结果同步给 User Model
                    # if len(_req.messages) > len(_req.user_messages):
                    #     new_context = _req.messages[len(_req.user_messages):]
                    #     for msg in new_context:
                    #         if msg['role'] == 'tool': # 如果是工具返回
                    #             # 伪装成 Agent 告诉用户
                    #             wrapped_msg = {
                    #                 "role": "assistant", 
                    #                 "content": f"I tried to access the database but got this error: {msg['content']}. Can you help?"
                    #             }
                    #             _req.user_messages.append(wrapped_msg)
                            
                    user_messages = [msg.model_dump() for msg in _req.user_messages]
                    
                    # Ensure the User Model knows it is the Customer (Assistant role in API)
                    # We do NOT swap roles because the Chat API generates the 'assistant' role.
                    # Agent = 'user', Customer = 'assistant'.
                    modified_messages = []
                    has_system = False
                    for msg in user_messages:
                        if msg['role'] == 'system':
                            has_system = True
                            new_msg = msg.copy()
                            # Prepend the instruction to be sure
                            new_msg['content'] = "IMPORTANT: You are playing the role of the USER (customer). The 'user' you are talking to is a support agent. You must act as the customer.\n\n" + new_msg['content']
                    ...
                    if not has_system:
                         modified_messages.insert(0, {
                             "role": "system", 
                             "content": "IMPORTANT: You are playing the role of the USER (customer). The 'user' you are talking to is a support agent. You must act as the customer."
                         })

                    # Disable thinking for user model to save context
                    user_response = await self._server_by_close_chat(modified_messages)
                    print(f'id: {_req.request_id} User response:{repr(user_response)}')
                except Exception as e:
                    logger.warning(f"req_id {_req.request_id} failed to get user response: {e}")
                    finish_reason_type = FinishReasonTypeEnum.STOP
                    break

                # 安全获取 content
                content = None
                if user_response and 'choices' in user_response and len(user_response['choices']) > 0:
                    message = user_response['choices'][0]['message']
                    content = message.get('content')
                    if not content:
                        logger.warning(f"req_id {_req.request_id} response content is empty. (Reasoning content present: {bool(message.get('reasoning_content'))})")
                ...
                if content:
                    ...
                    logger.warning(f"req_id {_req.request_id} failed to get user response or response content is empty. Response: {user_response}")
                    finish_reason_type = FinishReasonTypeEnum.STOP
                    break
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    for tool_call in parsed_tool_calls:
                        if tool_call.function.name in tool_call_times_by_tool:
                            tool_call_times_by_tool[tool_call.function.name] += 1
                    ...
                    for tool_call in parsed_tool_calls:
                        try:
                            result = await self._tool_map[tool_call.function.name].execute(
                                ...
                                shared_data = _req.taubench_database,
                                **_req.tools_kwargs[tool_call.function.name].get("execute_kwargs", {}),
                            )
                        except Exception as e:
                            result = (f"Error: {e}", 0.0, {})
                        logger.warning(f"{_req.request_id} {tool_call.function.name} result: {repr(result)}")
                        tool_call_results.append(result)

                    logger.warning(f"{_req.request_id} tool_call_results: {repr(tool_call_results)}")
                    ...
                    if len(_req.input_ids) >= self.config.max_model_len:
                        ...
                        break
                    # for tool_call in parsed_tool_calls:
                    if parsed_tool_calls[0].function.name in self.terminal_tools:
                        _req.state = AsyncRolloutRequestStateEnum.COMPLETED
                        finish_reason_type = FinishReasonTypeEnum.STOP
                        break
                    _req.state = AsyncRolloutRequestStateEnum.RUNNING
                ...
                content = output["text"]
                print(f"req_id {_req.request_id}: assistant response: {repr(content)}")
                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    ...
                    # --- 新增：判定内容是否带有工具调用标签 ---
                    import re
                    has_tool_tag = bool(re.search(r"<tool[_ ]?call", content, re.IGNORECASE))
                    ...
                    if self._function_call_parser and self._function_call_parser.has_tool_call(content):
                        ...
                        logger.warning(f"req_id {_req.request_id}: parsed_tool_calls: {parsed_tool_calls}")
                        ...
                        #     break
                            ...
                            if has_tool_tag:
                                ...
                                # 向模型说明原因
                                feedback = (
                                    "Error: Invalid tool call format. Your JSON has a syntax error "
                                    "(e.g., extra parenthesis ')' or malformed syntax). "
                                    "Please strictly follow the JSON format and try again."
                                )
                                _req.add_user_message(self.tokenizer, feedback)
                                # 标记格式错误供奖励函数识别
                                _req.update_metrics({'format_error': 1}, "system")
                                _req.state = AsyncRolloutRequestStateEnum.RUNNING
                                current_turns += 1
                                continue # 重视：这里不再 break，而是让模型纠错
                    else:
                        _req.state = AsyncRolloutRequestStateEnum.USER
                        _req.add_assistant_message(self.tokenizer, content)

        if current_turns >= self.config.multi_turn.max_turns:
            finish_reason_type = FinishReasonTypeEnum.STOP

        _req.rollout_turns = current_turns
        _req.tool_call_times = tool_call_count  # total tool calls
        _req.tool_call_times_by_tool = tool_call_times_by_tool 
        logger.info(f"req_id {_req.request_id} end for {finish_reason_type}, full_message: {_req.messages}")
    ...
    async def _handle_engine_call(self, _req: AsyncRolloutRequest, do_sample: bool, is_validate: bool, override_n: bool = True, **kwargs) -> dict:
        ...
        if self.config.response_length_one_turn:
        ...
        for key, value in kwargs.items():
            if key in temp_sampling_params:
                temp_sampling_params[key] = value
        # logger.info(f"req_id {_req.request_id} is_validate:{is_validate} do_sample:{do_sample} sampling_params:{temp_sampling_params}")
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for req in sorted_output_req_list:
            ...
            taubench_database.append(req.taubench_database)
        ...
        tool_call_times = []
        tool_call_times_by_tools = {tool_name: [] for tool_name in self._tool_map}
        rollout_turns = []

        for req in sorted_output_req_list:
            tool_call_times.append(req.tool_call_times)
            rollout_turns.append(req.rollout_turns)
            for tool_name, tool_call_times_per_tool in req.tool_call_times_by_tool.items():
                tool_call_times_by_tools[tool_name].append(tool_call_times_per_tool)
        ...
        batch = TensorDict(
            {
                ...
                "position_ids": position_ids,
                "loss_mask": loss_mask,
                "tool_call_times": torch.tensor(tool_call_times, dtype=torch.long),
                "rollout_turns": torch.tensor(rollout_turns, dtype=torch.long),
                **(
                    {
                        f'tool_call_times_{tool_name}': torch.tensor(tool_call_times_per_tool, dtype=torch.long)
                        for tool_name, tool_call_times_per_tool in tool_call_times_by_tools.items()
                    }
                )
            },
        ...
        return DataProto(
            ...
            non_tensor_batch={
                ...
                "taubench_database": np.array(taubench_database)
            },
        )
    ...
    def _preprocess_prompt_to_async_rollout_requests(self, prompts: DataProto, n: int) -> list[AsyncRolloutRequest]:
        ...
        req_list = []
        for data_idx, (raw_prompt, user_prompt) in enumerate(zip(prompts.non_tensor_batch["raw_prompt"], prompts.non_tensor_batch['user_prompt'])):
            for rollout_offset in range(n):
                if self._tool_schemas:
                    _tools_kwargs = prompts.non_tensor_batch["tools_kwargs"][data_idx]
                    _tool_schemas = [self._tool_map[k].get_openai_tool_schema() for k,v in _tools_kwargs.items() if v is not None]
                    # print(f"_tool_schema: {repr(_tool_schemas)}")
                    _input_ids = None
                ...
                # safe process raw_prompt
                if hasattr(raw_prompt, 'tolist'):
                    messages = raw_prompt.tolist()
                else:
                    messages = raw_prompt  

                if hasattr(user_prompt, 'tolist'):
                    user_messages = user_prompt.tolist()
                else:
                    user_messages = user_prompt  
                
                # load data
                ability = prompts.non_tensor_batch["ability"][data_idx]
                taubench_database = environment_manager.get_new_environment_data(ability)
                
                if taubench_database is None:
                    raise ValueError(f"No environment data found for ability: {ability}")
                ...
                req = AsyncRolloutRequest(
                    ...
                    taubench_database=taubench_database
                )
