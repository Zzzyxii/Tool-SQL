...
def _safe_model_dump(obj):
    """Return a dict for model-like objects, or pass through dict/str."""
    try:
...
class AsyncRolloutRequest(BaseModel):
    ...
    tool_call_times_by_tool: Dict[str, int] = {}
    rollout_turns: int = 0  # add
    enable_thinking: bool = True  # add
    metrics: Dict[str, List[Any]] = {}
    taubench_database: Dict[str, Any] = {}
    ...
    def initialize_request(cls, values):
        ...
        if not (user_messages := values.get("user_messages")):
            raise ValueError("user_messages is required for AsyncRolloutRequest initialization")

        enable_thinking = values.get("enable_thinking", True)

        values["messages"] = [Message.model_validate(msg) for msg in messages]
        values["user_messages"] = [Message.model_validate(msg) for msg in user_messages]

        tools = [_safe_model_dump(tool) for tool in tool_schemas] if (tool_schemas := values.get("tool_schemas", [])) else None
        ...
        if not values.get("input_ids") or not values.get("attention_mask"):
            tokenization_dict_with_prompt = tokenizer.apply_chat_template(
                messages,
                tools=[_safe_model_dump(tool) for tool in tool_schemas],
        ...
        values["base_conv_wo_gen_prompt_end_pos"] = len(
            tokenizer.apply_chat_template(BASE_CHAT_HISTORY, tools=tools, add_generation_prompt=False, tokenize=False)
        )
        values["base_conv_with_gen_prompt_end_pos"] = len(
            tokenizer.apply_chat_template(
                BASE_CHAT_HISTORY, tools=tools, add_generation_prompt=True, tokenize=False, enable_thinking=enable_thinking
            )
        )
        return values
    ...
    def get_generation_prompt_ids(self, tokenizer: PreTrainedTokenizer) -> list[int]:
        ...
        if self.use_inference_chat_template:
            chat_template_kwargs = {
                "conversation": [_safe_model_dump(msg) for msg in self.messages],
                "tools": [_safe_model_dump(tool) for tool in self.tool_schemas] if self.tool_schemas else None,
                "add_generation_prompt": True,
                "tokenize": True,
                "enable_thinking": self.enable_thinking,
            }
            # Debug
            # log_kwargs = chat_template_kwargs.copy()
            # log_kwargs["tokenize"] = False
            # print(f"req_id: {self.request_id} asistant input: {repr(tokenizer.apply_chat_template(**log_kwargs))}")
            return tokenizer.apply_chat_template(**chat_template_kwargs)
        else:
            # print(f"req_id: {self.request_id} asistant input: {repr(tokenizer.decode(self.input_ids))}")
    ...
    def add_assistant_message(
        ...
        # Append assistant message and update tokenization state
        if tool_calls is None:
            self.user_messages.append(Message(role="user", content=content))
        self.messages.append(Message(role="assistant", content=content, tool_calls=tool_calls))

        # Build content with chat template and encode the newly added assistant content
    ...
    def add_tool_response_messages(self, tokenizer: PreTrainedTokenizer, contents: list[str]) -> None:
        if not contents:
            return
        # Add tool messages and update input ids for tool responses (masked out)
    ...
    def add_user_message(self, tokenizer: PreTrainedTokenizer, content: str) -> None:
        # Add user message and update tokenization
        self.user_messages.append(Message(role="assistant", content=content))
    ...
    def finalize(
        ...
        if self.enable_tokenization_sanity_check:
            full_tokens = tokenizer.apply_chat_template([_safe_model_dump(msg) for msg in self.messages], tools=([_safe_model_dump(tool) for tool in self.tool_schemas] if self.tool_schemas else None), add_generation_prompt=False, tokenize=True)
